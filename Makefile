...
.PHONY : ntime_max.inc.FORCE
ntime_max.inc.FORCE:

$(SRC_DIR)/ntime_max.inc : ntime_max.inc.FORCE
	@echo "        INTEGER*4 NTIME_MAX ! Generated by Makefile" > $@.tmp
	@echo "        PARAMETER (NTIME_MAX = $(IRBEM_NTIME_MAX) ) ! Generated by Makefile. Set IRBEM_NTIME_MAX to override." >> $@.tmp
	@cmp -s $@.tmp $@ || (echo "Creating $@ with NTIME_MAX=$(IRBEM_NTIME_MAX). Set IRBEM_NTIME_MAX to override." ; mv $@.tmp $@)
	@rm -f $@.tmp
...
$(BIN_DIR)/%.o : $(SRC_DIR)/%.f $(SRC_DIR)/ntime_max.inc
	$(FC) $(FFLAGS) -c -o $@ $<
